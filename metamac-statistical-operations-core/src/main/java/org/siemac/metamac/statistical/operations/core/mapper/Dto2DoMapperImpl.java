...
public class Dto2DoMapperImpl implements Dto2DoMapper {
    ...
    private ExternalItem externalItemDtoToEntity(ExternalItemDto source, ExternalItem target, String metadataName) throws MetamacException {
        ...
        if (target == null) {
            target = new ExternalItem(source.getCode(), source.getUri(), source.getUrn(), source.getType(), internationalStringToEntity(source.getTitle(), null, metadataName), source.getManagementAppUrl());
        } else {
            target.setCode(source.getCode());
